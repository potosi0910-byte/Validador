...
def extraer_autorizaciones_rips(data, nombre_archivo=""):
    ...
    for usuario in data.get("usuarios", []):
        ...
        # ── Extraer procedimientos (urgencias y/o hospitalización) ──────────
        if tiene_urg or tiene_hosp:
...
def validar_autorizaciones(pacientes_rips, registros_excel, set_aut_excel):
    ...
    alertas = {
        ...
        'hosp_proc_cod_no_cruza': [], # (hosp+proc) codProcedimiento < 870000 no coincide en Excel
    }
    ...
    for num_doc, p in pacientes_rips.items():
        ...
        if tipo_atencion == 'hospitalario':
            ...
            # ── Detectar: RIPS con procedimientos Y hospitalizacion ──────
            # Si el paciente tiene AMBOS arrays y al menos un codProcedimiento
            # < 870000, se ejecuta ÚNICAMENTE la regla de cruce (cod + auth)
            # contra Excel. Todas las demás reglas hospitalarias se omiten.
            _tiene_proc_lt_870 = False
            if tiene_hosp and procedimientos_pac:
                for _p in procedimientos_pac:
                    try:
                        if int(_p.get('cod', '')) < 870000:
                            _tiene_proc_lt_870 = True
                            break
                    except (ValueError, TypeError):
                        pass

            if _tiene_proc_lt_870:
                # SOLO esta regla: cruce (numAutorizacion + codProcedimiento) en Excel.
                # Las demás reglas hospitalarias NO se ejecutan para este paciente.
                vistos_hosp_pc = set()
                for proc in procedimientos_pac:
                    ...
                    if not cod_proc or not na_proc:
                        continue
                    try:
                        if int(cod_proc) >= 870000:
                            continue  # >= 870000 no requiere validación cruzada
                    except (ValueError, TypeError):
                        continue
                    # Buscar el par (auth + cod) en la BD Excel
                    par_valido = False
                    if na_proc in auths_excel_pac:
                        codigos_auth = auths_excel_pac[na_proc]['codigos']
                        # Sin códigos registrados en Excel → se acepta (dato incompleto)
                        if not codigos_auth or cod_proc in codigos_auth:
                            par_valido = True
                    if not par_valido:
                        clave_pc = (num_doc, cod_proc, na_proc)
                        if clave_pc not in vistos_hosp_pc:
                            vistos_hosp_pc.add(clave_pc)
                            alertas['hosp_proc_cod_no_cruza'].append({
                                ...
                                'archivo_eps':  (auths_excel_pac[na_proc]['archivo']
                                                 if na_proc in auths_excel_pac else archivo_eps_def),
                                'mensaje': (
                                    "Validar el numero de autorizacion y el codigo "
                                    "autorizados por la EPS no coinciden con el RIPS json."
                                ),
                            })
                continue  # Saltar todas las demás reglas hospitalarias para este paciente
            ...
            if tiene_urg:
                for proc in procedimientos_pac:
                    ...
                    # Procedimiento < 870000: verificar que tenga auth en Excel
            ...
            # Se verifica contra set_aut_excel (TODOS los registros del Excel,
            # sin filtro por paciente) porque las autorizaciones de INTERNAC u
            # otros servicios pueden estar registradas en el Excel bajo un
            # documento distinto (p.ej. la madre del recién nacido).
            for na in set_auths:
                if to_int(na) <= auth_urgencias_int:
                    continue
                if na not in set_aut_excel:
...
def index():
    ...
    if request.method == 'POST':
        ...
        stats = {
            ...
            'hosp_proc_cod_nc':     len(alertas['hosp_proc_cod_no_cruza'])    if alertas else 0,
